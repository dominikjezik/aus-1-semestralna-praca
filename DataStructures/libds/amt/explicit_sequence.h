...
namespace ds::amt {
    ...
    bool ExplicitSequence<BlockType>::equals(const AMT& other)
	{
		...
		const ExplicitSequence* otherExplicitSequence = dynamic_cast<const ExplicitSequence*>(&other);
		...
		BlockType* myActual = this->accessFirst();
		BlockType* otherActual = otherExplicitSequence->accessFirst();

		while (myActual != nullptr)
		{
			// takto, lebo by sme museli dorobit != operator
			if (!(myActual->data_ == otherActual->data_))
			{
				return false;
			}

			myActual = this->accessNext(*myActual);
			otherActual = otherExplicitSequence->accessNext(*otherActual);
		}
	...
	{
		// TODO 04
		// ak do [] dame iba & posle sa vsetko co pozname ako referenciu
		// ak = posle vsetko ale kopiju

		/*
		size_t index = 0;
		auto ptr = findBlockWithProperty([&] (BlockType* block)->bool
		{
			if (block == &data)
			{
				return true;
		...
		return ptr == nullptr ? INVALID_INDEX : index;
		*/
		size_t result = 0;
	...
	{
		// TODO 04
		BlockType* blockToDelete = accessNext(block);

		if (blockToDelete == last_)
		{
			removeLast();
		}
		else
		{
			disconnectBlock(blockToDelete);
			AbstractMemoryStructure<BlockType>::memoryManager_->releaseMemory(blockToDelete);
		}
	}
	...
	{
		// TODO 04
		BlockType* blockToDelete = this->accessPrevious(block);

		if (blockToDelete == first_)
		{
			this->removeFirst();
		}
		else
		{
			this->disconnectBlock(blockToDelete);
			AbstractMemoryStructure<BlockType>::memoryManager_->releaseMemory(blockToDelete);
		}

...
namespace ds::amt {
	...
	public:
		...
		//---------- SEMESTRALNA PRACA - Iterator hierarchie uzemnych jednotiek ----------

		class LevelAccessHierarchyIterator :
			public PreOrderHierarchyIterator
		{
		public:
			LevelAccessHierarchyIterator(Hierarchy<BlockType>* hierarchy, BlockType* node);
			LevelAccessHierarchyIterator(const LevelAccessHierarchyIterator& other);
			void goToParent();
			bool canGoToParent();
			void goToSon(size_t sonOrder);

			// iteratory na sekvenciu blokov synov daneho nodu
			auto beginSons();
			auto endSons();

		private:
			void resetPositionsSequence();
		};
		...
		LevelAccessHierarchyIterator beginLevel();
		LevelAccessHierarchyIterator endLevel();
    };
    ...
    bool Hierarchy<BlockType>::isNthSon(const BlockType& node, size_t sonOrder) const
	{
		...
		return parent != nullptr
			&& accessSon(*parent, sonOrder) == &node;
	}
    ...
    void Hierarchy<BlockType>::processPreOrder(const BlockType* node, std::function<void(const BlockType*)> operation) const
	{
		...
		size_t kolkoSomSpracoval = 0;
		size_t kolkoMamSpracovat = degree(*node);
		size_t i = 0;

		while (kolkoSomSpracoval < kolkoMamSpracovat)
		{
			auto syn = accessSon(*node, i);
			if (syn != nullptr)
			{
				processPreOrder(syn, operation);
				++kolkoSomSpracoval;
			}
			++i;
		}
    ...
    void Hierarchy<BlockType>::processPostOrder(BlockType* node, std::function<void(BlockType*)> operation) const
	{
		...
		size_t kolkoSomSpracoval = 0;
		size_t kolkoMamSpracovat = this->degree(*node);
		size_t i = 0;

		while (kolkoSomSpracoval < kolkoMamSpracovat)
		{
			BlockType* syn = this->accessSon(*node, i);
			if (syn != nullptr)
			{
				this->processPostOrder(syn, operation);
				++kolkoSomSpracoval;
			}
			++i;
		}
	...
	//---------- SEMESTRALNA PRACA - Iterator hierarchie uzemnych jednotiek ----------
	template<typename BlockType>
	Hierarchy<BlockType>::LevelAccessHierarchyIterator::LevelAccessHierarchyIterator(Hierarchy<BlockType>* hierarchy, BlockType* node) :
		Hierarchy<BlockType>::PreOrderHierarchyIterator::PreOrderHierarchyIterator(hierarchy, node)
	{
	}

	template<typename BlockType>
	Hierarchy<BlockType>::LevelAccessHierarchyIterator::LevelAccessHierarchyIterator(const LevelAccessHierarchyIterator& other) :
		Hierarchy<BlockType>::PreOrderHierarchyIterator::PreOrderHierarchyIterator(other)
	{
	}

	template <typename BlockType>
	void Hierarchy<BlockType>::LevelAccessHierarchyIterator::goToParent()
	{
		auto nextNode = static_cast<BlockType*>(this->currentPosition_->currentNode_->parent_);
		if (nextNode != nullptr)
		{
			this->resetPositionsSequence();
			this->savePosition(nextNode);
		}
	}

	template <typename BlockType>
	bool Hierarchy<BlockType>::LevelAccessHierarchyIterator::canGoToParent()
	{
		auto nextNode = static_cast<BlockType*>(this->currentPosition_->currentNode_->parent_);
	...
	template <typename BlockType>
	void Hierarchy<BlockType>::LevelAccessHierarchyIterator::goToSon(size_t sonOrder)
	{
		auto currentNode = this->currentPosition_->currentNode_;
		auto nextNode = currentNode->sons_->access(sonOrder);
		if (nextNode != nullptr)
		{
			this->resetPositionsSequence();
			this->savePosition(static_cast<BlockType*>(nextNode->data_));
		}
	}

	template <typename BlockType>
	auto Hierarchy<BlockType>::LevelAccessHierarchyIterator::beginSons()
	{
		return this->currentPosition_->currentNode_->sons_->begin();
	}

	template <typename BlockType>
	auto Hierarchy<BlockType>::LevelAccessHierarchyIterator::endSons()
	{
		return this->currentPosition_->currentNode_->sons_->end();
	}


	template <typename BlockType>
	typename Hierarchy<BlockType>::LevelAccessHierarchyIterator Hierarchy<BlockType>::beginLevel()
	{
		return LevelAccessHierarchyIterator(this, accessRoot());
	}

	template <typename BlockType>
	typename Hierarchy<BlockType>::LevelAccessHierarchyIterator Hierarchy<BlockType>::endLevel()
	{
		return LevelAccessHierarchyIterator(this, nullptr);
	}

	template <typename BlockType>
	void Hierarchy<BlockType>::LevelAccessHierarchyIterator::resetPositionsSequence()
	{
    ...
    typename Hierarchy<BlockType>::PostOrderHierarchyIterator& Hierarchy<BlockType>::PostOrderHierarchyIterator::operator++()
	{
		...
		if (!this->currentPosition_->currentNodeProcessed_
			&& this->tryFindNextSonInCurrentPosition())
		{
			this->savePosition(this->currentPosition_->currentSon_);
			++(*this);
		}

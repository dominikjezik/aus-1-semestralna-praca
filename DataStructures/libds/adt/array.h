...
namespace ds::adt {
	...
	ADT& Array<T>::assign(const ADT& other)
	{
		...
		{
			this->error("AUT is different type.");
		}

		if (base_ != otherArray->base_ || size() != otherArray->size())
		{
			this->error("Dimmentions are different");
		}
	...
	T CompactMatrix<T>::access(long long index1, long long index2) const
	{
		...
		auto mappedIndex = mapIndices(index1, index2);
		return getSequence()->access(mappedIndex)->data_;
	}
	template<typename T>
	void CompactMatrix<T>::set(T element, long long index1, long long index2)
	{
		...
		auto mappedIndex = mapIndices(index1, index2);
		getSequence()->access(mappedIndex)->data_ = element;
	}

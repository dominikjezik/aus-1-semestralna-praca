...
namespace ds::adt {
    ...
    size_t GeneralList<T, SequenceType>::calculateIndex(T element)
    {
        // TODO 07
        size_t index = 0;
        auto block = this->getSequence()->findBlockWithProperty([&](typename SequenceType::BlockType* b)
        {
	        if (b->data_ == element)
	        {
                return true;
    ...
    {
        // TODO: v pseudo kode je opacna rovnost?
    ...
    {
        // TODO 07
        auto block = getSequence()->access(index);
        if (block == nullptr)
        {
    ...
    void GeneralList<T, SequenceType>::insert(T element, size_t index)
    {
        ...
        if (index > this->size())
        {
            this->error("Invalid index in list!");
        }
